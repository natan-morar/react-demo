...
function FormField(props: { element: Frontier.Element; onChange: (id: string, value: any, valid: boolean) => any }) {
    ...
    const genInput = () => {
        switch (props.element.type) {
            case 'boolean':
                return <BooleanInput element={props.element} onChange={onChange}></BooleanInput>;

            case 'text':
                return <Text element={props.element} onChange={onChange}></Text>;

            case 'textarea':
                return <Textarea element={props.element} onChange={onChange}></Textarea>;

            case 'multichoice':
                return <MultiChoice element={props.element} onChange={onChange}></MultiChoice>;
        }
    };
